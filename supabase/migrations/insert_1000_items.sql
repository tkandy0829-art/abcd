...
DECLARE
    ...
    v_img TEXT;
    -- 보물 이름 모음
    food_names TEXT[] := ARRAY['제주 귤', '으뜸 소고기', '흙에서 난 수박', '단 포도', '맛 좋은 복숭아', '손으로 만든 과자 꾸러미', '찬물로 우린 차', '조각 빵', '고기 모듬', '신선한 달걀 30알'];
    item_names TEXT[] := ARRAY['전자판', '누비판', '나무 책상', '글월 전집', '소리 가리개', '옛날 빛그림기', '들살이 화로', '나무 탁자', '작은 비춤틀', '달리는 발판'];
    categories TEXT[] := ARRAY['먹을거리', '번개틀', '옷가지', '놀거리', '살림살이'];
BEGIN
    -- 1. 음식 100개 인서트
    FOR i IN 1..100 LOOP
