...
const Auth: React.FC<AuthProps> = ({ onLogin, onRegister }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!/^[A-Za-z0-9]+$/.test(id) || !/^[A-Za-z0-9]+$/.test(pw)) {
      alert('아이디와 비밀번호는 영어와 숫자만 사용 가능합니다.');
      return;
    }
    if (isLogin) {
      onLogin(id, pw);
  ...
  return (
    <div className="min-h-screen bg-[#f3f4f6] flex items-center justify-center p-6">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl p-8 flex flex-col items-center">
        <CarrotLogo />
        <h1 className="text-2xl font-bold text-gray-800 mt-4 mb-2">당근 중고거래 시뮬</h1>
        <p className="text-gray-500 text-sm mb-8">{isLogin ? '계정에 로그인하세요' : '새로운 상인이 되어보세요'}</p>

        <form onSubmit={handleSubmit} className="w-full space-y-4">
          <div>
            <label className="block text-xs font-semibold text-gray-400 mb-1 ml-1 uppercase">ID</label>
            <input 
              type="text" 
              value={id}
              ...
              placeholder="영어/숫자 아이디"
              required
            />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-400 mb-1 ml-1 uppercase">Password</label>
            <input 
              type="password" 
              value={pw}
              onChange={(e) => setPw(e.target.value)}
              className="w-full px-4 py-3 rounded-xl border border-gray-100 focus:border-[#ff8a3d] focus:ring-1 focus:ring-[#ff8a3d] outline-none transition-all"
              placeholder="영어/숫자 비밀번호"
              required
            />
          </div>
          <button 
