...
const Sell: React.FC<SellProps> = ({ user, onUpdateUser, onBack, onNegotiationUpdate }) => {
  ...
  const completeTransaction = () => {
    ...
    try {
      ...
      alert(`판매 완료! ${finalPrice.toLocaleString()}원을 받았습니다.`);
      ...
      <div className="flex flex-col h-full bg-gray-50">
        <div className="p-4 bg-white border-b flex justify-between items-center shadow-sm">
          ...
          <div className="text-center flex-1 mx-4">
            ...
            <p className="text-[10px] text-gray-400 uppercase tracking-widest">구매자</p>
          </div>
          ...
          {hasError && (
            <div className="flex justify-center my-4">
              <div className="bg-red-50 text-red-600 px-4 py-3 rounded-2xl text-[11px] font-bold border border-red-100 shadow-sm text-center">
                ⚠️ 서버가 바빠서 대답을 못하고 있어요 (과부하).<br />
                잠시 기다리거나, "네 알겠습니다"를 눌러 현재 가격으로 판매를 완료할 수 있습니다.
              </div>
            </div>
          )}
        </div>
        ...
        <div className="fixed bottom-20 left-1/2 -translate-x-1/2 w-full max-w-md bg-white p-3 border-t flex flex-col gap-2 shadow-[0_-10px_20px_-5px_rgba(0,0,0,0.05)]">
          <div className="flex justify-between items-center px-2 mb-1">
            <p className="text-xs text-gray-400 mt-1">이웃과 채팅으로 가격을 협상합니다.</p>
            <span className="text-xs text-gray-400 font-medium tracking-tight">현재 제시 가격</span>
          ...
          <div className="flex gap-2">
            <input
              ...
              disabled={isLoading}
              placeholder={isLoading ? "이웃의 답변을 기다리는 중..." : "얼마에 팔고 싶으신가요?"}
          ...
          <p className="text-[10px] text-gray-400 text-center mt-1">"네 알겠습니다"라고 입력하면 현재 가격에 판매가 완료됩니다.</p>
        </div>
      </div>
    );
  }
          ...
          {user.inventory.map((item: Item) => (
            <div key={item.id} className="bg-white border border-gray-100 p-4 rounded-2xl shadow-sm flex gap-4 items-center active:scale-[0.98] transition-transform">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-orange-500 font-bold">예상 가격: {(Number(item.basePrice * (item.isCleaned ? 2 : 1)) || 0).toLocaleString()}원</p>
              </div>
